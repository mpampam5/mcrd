...
if (isset($_POST['generate']))
{
    ...
    if ($table_name <> '')
    {
        ...
        if ($mvc=="mvc") {
          $direct_created      = "";
          $direct_created_view = "views/$c_url/";
          if (!file_exists($target."".$direct_created_view))
           {
               mkdir($target."".$direct_created_view, 0777, true);
           }
        }else {
          $direct_created      = "modules/$c_url/";
          $direct_created_view = "modules/$c_url/views/";

          if (!file_exists($target."".$direct_created))
           {
               mkdir($target."modules/".$c_url, 0777, true);
               mkdir($target."modules/".$c_url."/controllers", 0777, true);
               mkdir($target."modules/".$c_url."/models", 0777, true);
               mkdir($target."modules/".$c_url."/views", 0777, true);
           }
        }
