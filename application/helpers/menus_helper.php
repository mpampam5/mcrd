  ...
  function menu()
  {
    ...
    foreach ($menu->result() as $menus) {
      ...
      if ($submenu->num_rows() > 0) {
        ...
        $str .= "<ul class='treeview-menu'>";
            foreach ($submenu->result() as $submenus) {
              if ($ci->cl->get_menus($submenus->id)==true) {
                  $str .= "<li class='".menus_active($submenus->link)."'><a href='".site_url($submenus->link)."'><i class='".$submenus->icon."'></i><span>".strtoupper($submenus->name)."</span></a></li>";
              }
            } //end foreach submenu
        $str .= "</ul></li>";
      }
        ...
        if ($ci->cl->get_menus($menus->id)==true) {
          $str .= "<li class='".menus_active($menus->link)."'><a href='".site_url($menus->link)."'><i class='".$menus->icon."'></i><span>".strtoupper($menus->name)."</span></a></li>";
        }
      } //end if sub menu
    } //end foreach menu
